      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <motion.div variants={staggerContainer(0.1, 0)} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-50px' }} className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {skillsData.map((category, idx) => {
            ...
            return (
              <motion.div key={category.id} variants={fadeIn('up', 0)} className={`rounded-3xl bg-white border border-blue-100 p-8 space-y-6 shadow-sm hover:shadow-xl hover:shadow-blue-100/50 hover:border-blue-200 transition-all duration-300`}>
        ...
        <motion.div variants={fadeIn('up', 0)} initial="hidden" whileInView="show" viewport={{ once: true }} className="mt-16 text-center">
          <p className="text-saey-muted text-sm mb-6 tracking-wider uppercase">{t.skills.toolsLabel}</p>
          <div className="flex flex-wrap justify-center gap-3">
            {['Meta Ads Manager','Adobe Photoshop','Adobe Illustrator','Adobe Premiere Pro','After Effects','CapCut','Canva','Figma','Meta Business Suite','Google Analytics','Next.js','Database','SQL Server','Node.js'].map((tool) => (
              <span key={tool} className="px-4 py-2 rounded-xl text-sm font-medium bg-saey-gray border border-blue-100 text-saey-muted hover:text-saey-navy hover:border-blue-300 hover:bg-blue-50 transition-all duration-200 cursor-default">{tool}</span>
            ))}
          </div>
        </motion.div>
      </div>
