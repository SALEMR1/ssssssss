    ...
    <section id="results" className="relative py-32 bg-saey-gray overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          {statsData.map((stat, idx) => {
            ...
            return (
              <motion.div key={stat.id} variants={fadeIn('up', 0)} whileHover={{ y: -6 }}
                className={`group relative rounded-3xl overflow-hidden border border-blue-100 bg-white hover:border-blue-300 hover:shadow-xl hover:shadow-blue-100/50 transition-all duration-500`}>
                ...
                <div className="relative p-8 space-y-4">
                  ...
                  <p className="text-saey-navy font-semibold">{label}</p>
        ...
        <motion.div variants={fadeIn('up', 0)} initial="hidden" whileInView="show" viewport={{ once: true }}
          className="mt-16 p-8 rounded-3xl border border-blue-100 bg-white shadow-sm text-center">
          <p className="text-saey-muted text-lg leading-relaxed max-w-3xl mx-auto">
            {t.results.context}{' '}<span className="text-saey-navy font-semibold">{t.results.clients}</span>{' '}{t.results.context2}
          </p>
        </motion.div>
      </div>
    </section>
